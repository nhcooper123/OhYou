...
% Weird issue with endfloat means these need to have no white space
  ...
  \subsection{Recommendations for interpreting $\alpha$}    
    ...
    Often, when Likelihood ratio tests suggest the OU model should be favoured over the Brownian model, estimates of $\alpha$ are actually very close to zero (e.g., examples in \citealp{Harmon:2010aa}), i.e. very close to Brownian.
...
\section{Epilogue: Open and (vaguely) reproducible science}
  ...
  Although free to download, this software is not an Open Source, which has implications for long-term development of the methods.
