...
\label{section:introduction} 
  ...
  Uses of the OU model can be divided into three broad categories. % improve citations in this section? % fix up numbers. Argggggggghhhhhhhhhhh.
    \begin{enumerate}
  ...
  The majority of papers use the OU model to model the evolution of a continuous character (Fig 2 and Supplementary Material), % Need to add ref to supp mat
  ...
  The principles here also apply to a range of other macroevolutionary models that can be fit to continuous data and compared using model testing procedures.
    ...
    \begin{table}[h]
      \begin{centering}
        \caption{The most common uses of Ornstein Uhlenbeck models in ecology, evolutionary biology and palaeontology papers published between 2005 and 2013. See Supplementary Material for details linkto supp mat.}
        \bigskip  
...
\section{Materials and Methods}
  \subsection{The Ornstein Uhlenbeck (OU) model}
    ...
    Eventually, when $\alpha$ is really large, all imprint of history is lost and the trait evolution is essentially a rapid burst at the present (FIGURE 2). % Fix the figure or send to supplemental

    %$\alpha$ can most easily be interpreted by using it to estimate the ``phylogenetic half-life'' ($t_\frac{1}{2}$) of a trait, i.e., the time it takes for a species entering a new niche to evolve halfway toward its new expected optimum \citep{hansen1997stabilizing}, as follows:
    ...
    %If $t_\frac{1}{2}$ is short relative to the branch lengths of the phylogeny, adaptation is quick, residual phylogenetic correlations are weak, and there is little influence of the past. This is sometimes referred to as ``phylogenetic inertia'' but we dislike this term
...
%The point of highlighting these aspects of expected behavior is that we believe these properties need to be understood if we are to build up a full picture of when the model is likely to be well estimated, as well as to understand the bounds of what can be inferred from real data. 
  ...
  \label{section:sims.methods} 
    ...
    This procedure ... % explain WHY we are doing this, how does it relate to uses of the OU model.
