...
\keywords{OU - comparative methods - phylogeny - stabilising selection - macroevolutionary models} %meant to be after abstract
...
\section{Recommendations and potential solutions}
  ...
  \subsection{Recommendations for model fitting}
    \begin{enumerate}[(i)]
      ...
      Simulating data under the BM model will generate null distributions \citep[e.g.,][]{boettiger2012your} and allow straightforward assessment of model fit, for example by generating appropriate critical values for Likelihood ratio tests. %We could potentially add a script/workflow for this
      \citet{boettiger2012your} discuss this at length and we recommend reading their paper for a full overview of this approach.
      % NC: Though their method is hard to apply it does cover all the points we discuss here so we chould make it clearer (as requested by Syst Biol reviewers). Also I'd forgotten that the new geiger code means their package is deprecated and can't be used anyway.
...
% Weird issue with endfloat means these need to have no white space
  ...
  \subsection{Recommendations for interpreting $\alpha$}    
    ...
    The similarity between Brownian and OU models with small $\alpha$ is demonstrated in Figure XX. % NC: Not sure what this figure is...
...
% NC: I'm not so sure about this paragraph. It reads like a Freckleton paper and I'm not sure how helpful it is. I'd be tempted to just cut it and the subheading, and end with the paragraph on how it applies to all models. What do you think?
  ...
  % NC: this is a leftover from the previous version - is V the variance covariance matrix?
...
% NC: Slightly modified i think this is a good end paragraph. Perhaps with a bit more form the original conclusion below.
...
\section{Epilogue: (vaguely) reproducible science}
  ...
  % NC: I'm not sure what else to write here without pissing off Chris etc. Is it possible to put their scripts on GitHub? Can you ask? 
