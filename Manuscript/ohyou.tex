...
\label{section:introduction} 
  ...
  We are not the first to mention these problems, but we hope by summarising the issues and giving recommendations we can help those less familiar with the technical literature \citep[e.g.,][]{ho2013asymptotic,ho2014intrinsic,boettiger2012your}. %ives garland 2010, hansen bartoszek et al
...
\section{Materials and Methods}
  \subsection{What is the OU model?}
    ...
    $\alpha$ is sometimes referred to as the ``rubber band'' parameter because of the way it pulls traits back towards $\mu$. For more details see Appendix.
  ...
  \subsection{What do people use the OU model for?}
    The popularity of the OU model has grown extensively in recent years (Fig. \ref{figure.literature}), in part because these models are now easy to implement apply via packages in R \citep[e.g. ouch, GEIGER and OUwie;][]{Butler:2004aa,Harmon:2008aa,beaulieu2012ouwie}. 
    ...
    In our simulations (see below), we focus on the first of these because the principles apply to a range of other macroevolutionary models.  
     ...
     % OK somewhere here need to clarify that a) most ou studies are looking at fitting the model, and comparing models, not using multiple optima stuff. Then clarify that here we are focussing on that popular use of the 

     % Problems with the OU model and where they are reported in the literature?

     % This maybe belongs in the main text in shorter form  
...
% Still not sure of these two equations!
    \begin{equation}
      var(X_i) = \frac{(1 - e^{-2 \alpha T}) \times \sigma^2}{2 \alpha}
  ...
  And for a pair of species i and j, the covariance in traits is:
    ...
    \begin{equation}
      cov(X_i, X_j) = \frac{e^{-2 \alpha (T - t_{ij})} \times (1 - e^{-2\alpha t_{ij}})\times \sigma^2}{2 \alpha}
