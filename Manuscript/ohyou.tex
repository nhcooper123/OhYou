...
\section{The Ornstein Uhlenbeck (OU) model}
  ...
  Under this formulation, $\alpha$ can be considered as the strength of the pull towards the primary optima \citep{hansen2012adaptive} but not as an estimate of stabilizing selection in the population genetics sense. 
...
\section{Effects of measurement error on OU model performance}
  ...
  \end{landscape}
    ...
    Second, for moderate amounts of error (5-10\%), the ML estimates of $\alpha$ are consistently \textgreater 1. 
...
\begin{enumerate}[(i)]
  ...
  Our results indicate that a simple Brownian process can be mistaken for an OU process when a small amount of error is added to the data. 
...
\end{enumerate}
  ...
  This is particularly important for inference of phylogenetic niche conservatism. 
  ...
  A more parsimonious explanation would be that there is measurement error in the data.
  This seems particularly likely for climatic niche traits that are often averaged across an entire species range. 
  Concluding from data on extant species that traits are limited by an OU-like process will therefore always be a leap of faith. 
...
\subsection{Potential solutions and recommendations}
  Although there are limitations with the OU model, there are also several ways researchers can get around these problems as follows:
  
  \begin{enumerate}[(i)]
    \item \textbf{Use at least 200 species.} 
    If you really want to be confident that the OU model is favoured over the Brownian model, you need to fit the models to more than 200 species. 
    Note that this does not solve the issue of measurement error. in the data.
...
\end{enumerate}
  ...
  If $\alpha$ is small, then this should not be interpreted as any OU-like process (including phylogenetic niche conservatism etc.). 
