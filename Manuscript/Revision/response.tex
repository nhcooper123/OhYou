...
% Note that these issues will also apply when using the Akaike Information Criteria (AIC) to compare models, as this is essentially the same as performing a Likelihood ratio test. AIC also presents other difficulties when used for complex models such as those use in phylogenetic comparative analyses, so we tend to avoid them it these reasons. 
